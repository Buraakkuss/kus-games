...
PGDATA="$(mktemp -d /var/lib/postgresql/sukun-verify-XXXXXX)"
SOCK="$(mktemp -d)"
...
"${PSQL[@]}" -d postgres -c "create database sukun_test;" >/dev/null
"${PSQL[@]}" -d sukun_test -c "create extension if not exists pgcrypto;" >/dev/null
"${PSQL[@]}" -d sukun_test -f "$SUPA/tests/00_auth_stub.sql" >/dev/null

for f in "$SUPA"/migrations/*.sql; do
  printf '  migration %s\n' "$(basename "$f")"
  "${PSQL[@]}" -d sukun_test -f "$f" >/dev/null
done

echo "  RLS sınaması"
"${PSQL[@]}" -d sukun_test -f "$SUPA/tests/01_rls_test.sql"

# Kapsama kontrolü: politikasız RLS tablosu ya da RLS'siz tablo kalmasın (§71).
echo "  kapsama kontrolü"
"${PSQL[@]}" -d sukun_test -f "$SUPA/tests/02_rls_coverage.sql"
