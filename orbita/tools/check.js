...
console.log('\n1) Oyun betigi');
{
  ...
  if (!/function capture/.test(html)) bad('capture() kayip - adaletsiz inis korumasi orada');
...
console.log('\n5) Surum ve kimlik tutarliligi');
{
  ...
  ['index.html','privacy.html','terms.html','support.html'].forEach(f => {
    ...
    if (/nakitpilot|\bslot\b|\blatch\b/i.test(s)) bad('docs/' + GAME + '/' + f + ' icinde baska projeden kalan metin var');
  });
}
