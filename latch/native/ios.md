...
<string>Your choice here only affects how relevant the ads in Latch are. The game itself works exactly the same either way.</string>
...
| Basılı tutma algılanmıyor | WKWebView'de `pointerup` bazı durumlarda gelmez; oyun `pointercancel` olayını da dinliyor, bu satırı silme |
