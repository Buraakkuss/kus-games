...
$PB -c "Delete :NSLocationWhenInUseUsageDescription" "$PLIST" 2>/dev/null
...
$PB -c "Add :NSUserTrackingUsageDescription string 'Sana daha uygun reklamlar gosterebilmek icin izin istiyoruz. Reddedersen $APPNAME aynen calismaya devam eder.'" "$PLIST"
# Konum izni ZORUNLU DEGIL: sehir listesi ve elle koordinat girisi de var.
# Metin bunu soylemezse App Review "neden konum istiyorsunuz" diye geri cevirir.
$PB -c "Add :NSLocationWhenInUseUsageDescription string 'Namaz vakitlerini ve kible yonunu bulundugun yere gore hesaplamak icin konumun kullanilir. Konumun cihazindan cikmaz. Istersen izin vermeden sehir listesinden de secebilirsin.'" "$PLIST"
